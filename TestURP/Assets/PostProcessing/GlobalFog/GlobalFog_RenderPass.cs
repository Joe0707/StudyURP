...
public class GlobalFog_RenderPass : TK_RenderPassBase
{
    ...
    protected override void Render(CommandBuffer cmd, ref RenderingData renderingData)
    {
        ...
        cmd.GetTemporaryRT(TempTargetId, cameraData.camera.scaledPixelWidth, cameraData.camera.scaledPixelHeight, 0, FilterMode.Trilinear, RenderTextureFormat.Default);//设置目标贴图
        cmd.Blit(source, TempTargetId);     //将source复制到临时的一份RT上去
        cmd.Blit(TempTargetId, source, material, 0);    //临时RT经过处理之后，再传回source上
